...
export default function UsersPage() {
  ...
  const load = useCallback(async () => { const response = await fetch("/api/users"); if (response.ok) setUsers(await response.json()); else { setError("Solo un administrador puede ver esta página."); if (response.status === 401) router.replace("/login"); } }, [router]);
