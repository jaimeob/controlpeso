...
export default function Home() {
  ...
  const manualQuantity = Math.max(Number(manual.quantity) || 1, 0.1);
  const roundNutrition = (value: string) => Math.round(Number(value || 0) * manualQuantity * 10) / 10;
  const manualCaloriesTotal = Math.round(roundNutrition(manual.calories));
  ...
  async function addManually(event: FormEvent) {
    ...
    const food: Food = { id: editingFood?.id || crypto.randomUUID(), name: manual.name.trim(), portion: `${manual.quantity} ${manual.unit}`, quantity: manualQuantity, unit: manual.unit, calories: manualCaloriesTotal, protein: roundNutrition(manual.protein), carbs: roundNutrition(manual.carbs), fat: roundNutrition(manual.fat), createdAt: editingFood?.createdAt || new Date().toISOString(), source: editingFood?.source || "Manual" };
  ...
  function openManual(food?: Food) {
    ...
    const quantityForUnit = Math.max(Number(food?.quantity) || 1, 0.1);
    const perUnit = (value: number) => Math.round((value / quantityForUnit) * 10) / 10;
    setManual(food ? { name: food.name, quantity: quantityForUnit, unit: food.unit || "pieza", calories: String(perUnit(food.calories)), protein: String(perUnit(food.protein)), carbs: String(perUnit(food.carbs)), fat: String(perUnit(food.fat)) } : { name: "", quantity: 1, unit: "pieza", calories: "", protein: "", carbs: "", fat: "" });
    setManualOpen(true);
  }
  ...
  const canUseAi = session.roles.includes("superadmin");
    ...
    <main className="app-shell">
      ...
      <section className="content">
        ...
        <div className="dashboard-grid">
          <section className="hero-card"><div className="hero-copy"><div className="ai-label"><Icon name="sparkle" /> ASISTENTE IA</div><h2>¿Qué comiste hoy?</h2><p>{canUseAi ? "Indica el alimento, la cantidad y su unidad." : "El asistente de IA está reservado para superadministradores."}</p><form onSubmit={analyze} className="food-form"><div className="search-box"><Icon name="search" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={canUseAi ? "Ej. huevo cocido o pechuga" : "Asistente de IA bloqueado"} aria-label="Alimento" disabled={!canUseAi} /><button type="submit" disabled={loading || !canUseAi}>{loading ? "Analizando..." : "Analizar"}</button></div><div className="quantity-row"><label>Cantidad<input type="number" min="0.1" step="0.1" value={quantity} onChange={(event) => setQuantity(Number(event.target.value))} disabled={!canUseAi} /></label><label>Unidad<select value={unit} onChange={(event) => setUnit(event.target.value)} disabled={!canUseAi}><option value="pieza">Piezas</option><option value="gramos">Gramos</option><option value="mililitros">Mililitros</option></select></label></div></form>{!canUseAi && <p className="ai-locked-message">Puedes registrar tus alimentos con el botón “Agregar alimento”.</p>}{notice && <div className="notice"><Icon name="check" /> {notice}</div>}</div><div className="hero-orb"><span>✦</span><i>✦</i></div></section>
